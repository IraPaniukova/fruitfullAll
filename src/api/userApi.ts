import { API } from "./axios";

export const getUsers = () => API.get("/Users").then((res) => res.data); // GET all users (admin only)

export const getUserById = (id: number) =>
  API.get(`/Users/${id}`).then((res) => res.data); // GET single user by id

export const registerEmailUser = (userData: any) =>
  API.post("/Users", userData).then((res) => res.data); // POST new user (signup with an email)

export const updateUser = (id: number, userData: any) =>
  API.put(`/Users/${id}`, userData).then((res) => res.data); // PUT update user info

export const updateUserLogin = (id: number, loginData: any) =>
  API.put(`/Users/${id}/login`, loginData).then((res) => res.data); // PUT update login data

export const deleteUser = (id: number) =>
  API.delete(`/Users/${id}`).then((res) => res.data); // DELETE user by id
